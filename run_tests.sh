...
comm -23 <(cat logs/* | grep '.log$' | sed -r 's/^[0-9]{4}\-[0-9]{2}\-[0-9]{2} //;s/[]].*/]/' | sort) <(sudo tail /var/log/messages -n"$(($numb_lines*5))" | awk '{print $3" "$6" "$7}' | sort) || { echo "FAILED syslog timing and pid mismatch" >&2 ; ret="$((ret + $?))" ; }
