...
M_EXT_CONNS|Extensions: m||The list of connections. (overridden by running $M_EXT_CONN_SCRIPT if it is setup)<br />A connection is a HOST with optional DB, ENGINE, USER, and PASSWORD in that order (previous variables required for subsequent ones to be recognized and use the VCLOD_DST_ prefix with the normal defaulting behavior).<br />You may use any prefix or modify any env variable if you format the line such that ALL arguments define the variable name they are overriding.
...
PARA_EXT_OPERATION|Extensions: para|sh-$base_filename|what vclod_operation to use to process a line of stdin.<br />NOTE: When specifying this in the config, do NOT self-reference or you can produce a fork bomb.
