...
#include | Load a sql script file (with no .extension) to handle any additional reformatting or processing that is required. If added to a field line, also acts like #ignore.<br />If using .etlsql/.etldst, you can optionally add .extensions that will preform those operations with implicit starting and ending .sql/.dst extensions (dependind on whether you used .etlsql or .etldst respectively).
...
#addex | Add a column to the given #explode table name. Used inside a multiline comment (`/**/`), the field name must be followed by a field definition like in the main temp table.<br />Format: <field name/type> #addex <tmp_table_name>
#addex_index | Add an index to the given #explode table name. Used inside a multiline comment (`/**/`), the index may be 1) a single field name, 2) one or more field names inside parentheses, or 3) an index name followed by one or more field names inside parentheses.<br />Format: <index name and/or column list> #addex_index <tmp_table_name>
#addex_with_index | A logical union of the #addex and the first form of the #addex_index commands.<br />Format: <field name> <field type> #addex_with_index <tmp_table_name>
...
ui_split | Layout | tries its best to not bloat AUTO_INCREMENTING keys by being more particular about managing whether it uses an UPDATE or INSERT. Best with lower cardinality tables.<br />NOTE: CANNOT use with #join. Instead, use #addex to add the columns you need and #append to sync the 2 tables.
...
All Sub tables have an `id` column that links to the main temp table's id field (that the #explode command is attached to -- should not be called id). The 2 id fields are used for normal #sync operations by using the #join command. NOTE: These operations will preform faster when using the .etlsql/.etldst varients.
...
_manual | User Defined (must include id) | takes an #include file instead of a #generate SQL clause so you can define your own temp table to use. Be sure to keep the temp table's name consistent.<br />If using .etlsql/.etldst, you can optionally add .extensions that will preform those operations with implicit starting and ending .sql/.dst extensions (dependind on whether you used .etlsql or .etldst respectively).
